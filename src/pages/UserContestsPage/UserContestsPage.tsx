...
import { Plus, Heart, Star, Users, Clock, Coins, ArrowLeft, ChevronDown, Trash2, Pencil, Eye, EyeOff, Send, MessageSquare, ThumbsUp, Trophy, Share2 } from 'lucide-react';
...
export default function UserContestsPage() {
  ...
  const refresh = async () => {
    ...
    if (uid) {
      const { data: parts } = await supabase.from('custom_participants').select('*').eq('user_id', uid).not('contest_id','is',null);
      const ids = (parts ?? []).map((x:any)=>x.contest_id);
      const { data: ps } = ids.length ? await supabase.from('custom_contests').select('*').in('id', ids) : { data: [] };
      setJoined((ps ?? []).map((x:any)=>{ const rec = (parts??[]).find((y:any)=>y.contest_id===x.id); return {...x, _status: rec?.status, _recId: rec?.id}; }));
      const { data: u } = await supabase.from('user_meta').select('coins').eq('id', uid).single();
      setMyCoins(u?.coins || 0);
    }
  };

  const completeContest = async (p: any) => {
    if (!me) return;
  ...
  const rate = async (pid: string) => {
    ...
    const { data: parts } = await supabase.from('custom_participants').select('*').eq('contest_id', pid).eq('user_id', me);
    const part = parts && parts[0];
    if (!part) { alert('只有参加过该竞赛的用户可以评分'); return; }
    ...
    if (!abandoned && elapsed < days / 2) {
      alert('竞赛进行未过半（已进行 ' + elapsed.toFixed(1) + ' 天 / 共 ' + days + ' 天），需过半或中途放弃后才能评分');
      return;
    }
  ...
  const joinContest = async (p: any) => {
    if (!me) { alert('请先登录'); return; }
    if (p.author === me) { alert('不能参加自己发布的竞赛'); return; }
    const { data: exist } = await supabase.from('custom_participants').select('id').eq('contest_id', p.id).eq('user_id', me);
    ...
    if (!already) {
      await supabase.from('custom_participants').insert({ contest_id: p.id, user_id: me, status: 'active' });
      await supabase.from('custom_contests').update({ usage_count: (p.usage_count || 0) + 1 }).eq('id', p.id);
    }
    alert(already ? '你已参加过该竞赛（重复参加不重复计数）' : '报名成功！押金 ' + cost + ' 猫猫币，完成后退还并获得奖励');
    refresh();
  };

  const toggleLike = async (pid: string) => {
    if (!me) return;
    const { data } = await supabase.from('likes').select('id').eq('user_id',me).eq('target_type','contest').eq('target_id',pid);
    if (data && data.length) { await supabase.from('likes').delete().eq('id', data[0].id); }
    else { await supabase.from('likes').insert({user_id:me,target_type:'contest',target_id:pid}); }
  };

  const toggleFav = async (pid: string) => {
    if (!me) return;
    const { data } = await supabase.from('favorites').select('id').eq('user_id',me).eq('target_type','contest').eq('target_id',pid);
    if (data && data.length) { await supabase.from('favorites').delete().eq('id', data[0].id); }
    else { await supabase.from('favorites').insert({user_id:me,target_type:'contest',target_id:pid}); }
  };
  ...
  const save = async () => {
    ...
    if (editing) {
      if (editing.is_public !== false && (editing.usage_count || 0) > 0) {
        await supabase.from('custom_contests').update({ pending_changes: payload }).eq('id', editing.id);
        alert('该竞赛正在被参与，修改已进入赛季等待队列，赛季结算后自动生效。也可在卡片上点击「立即生效」花费猫猫币即时推送。');
      } else {
        await supabase.from('custom_contests').update(payload).eq('id', editing.id);
      }
    } else {
      if (form.isPublic) {
        const { data: u } = await supabase.from('user_meta').select('coins').eq('id', me).single();
        const cost = 50;
        if (!u || (u.coins || 0) < cost) { alert('上架公开竞赛需要 ' + cost + ' 猫猫币，余额不足'); return; }
        await supabase.from('user_meta').update({ coins: u.coins - cost }).eq('id', me);
      }
      await supabase.from('custom_contests').insert({ ...payload, author: me });
    }
  ...
  const pushNow = async (p: any) => {
    ...
    await supabase.from('user_meta').update({ coins: u.coins - cost }).eq('id', me);
    await supabase.from('custom_contests').update({ ...p.pending_changes, pending_changes: null }).eq('id', p.id);
    refresh();
  };

  const withdrawPending = async (p: any) => {
    await supabase.from('custom_contests').update({ pending_changes: null }).eq('id', p.id);
    refresh();
  };
  ...
  return (
    <div className="space-y-4">
      ...
      {tab === 'joined' && (
        <Card>
          <CardHeader><CardTitle>我参与的竞赛（猫猫币余额 {myCoins}）</CardTitle></CardHeader>
          <CardContent>
            <ScrollArea className="h-[400px]">
              {joined.map(p => (
                <div key={p.id} className="mb-3 rounded border p-3">
                  ...
                  {p._status === 'active' && (
                    <div className="mt-2 flex gap-2">
                      <Button size="sm" onClick={()=>completeContest(p)}>提交完成</Button>
                      <Button size="sm" variant="outline" onClick={()=>quitContest(p)}>放弃</Button>
                    </div>
                  )}
                </div>
              ))}
              {joined.length === 0 && <p className="text-sm text-muted-foreground">还没有参加任何竞赛，去「别人发布的」里看看吧</p>}
            </ScrollArea>
          </CardContent>
        </Card>
      )}
            ...
            <ScrollArea className="h-[400px]">
              {otherItems.map(p => (
                <div key={p.id} className="mb-3 rounded border p-3">
                  ...
                  <div className="mt-2 flex gap-2">
                    <Button size="sm" onClick={()=>joinContest(p)}><Users className="h-3 w-3" /> 报名</Button>
                    ...
                    <Button size="sm" variant="outline" onClick={()=>{navigator.clipboard.writeText(location.origin+'/career-lab/user-contests');alert('项目链接已复制，可分享给好友');}}><Share2 className="h-3 w-3" /> 转发</Button>
                  </div>
                </div>
              ))}
            </ScrollArea>
